...
`);
const stmtGetAllCases = db.prepare(`SELECT data FROM cases ORDER BY created_at DESC`);
...
async function startServer() {
  ...
  app.post('/api/cases', (req, res) => {
    try {
      ...
      if (!caseData?.id) {
        return res.status(400).json({ error: 'Case object must include an id field.' });
      }
      stmtInsertCase.run({
        id: caseData.id,
  ...
  app.post('/api/analyze-skin', async (req, res) => {
    try {
      ...
      form.set('image', new Blob([imageBuffer], { type: 'image/jpeg' }), 'image.jpg');
      ...
      console.log(`Forwarding to DermaVision → patient: ${patientInfo?.name || 'Anonymous'}`);
      ...
      if (!dermaResponse.ok) {
        ...
        console.error(`DermaVision error ${dermaResponse.status}: ${errorText}`);
        return res.status(502).json({ error: `DermaVision API error ${dermaResponse.status}: ${errorText}` });
      }

      const dermaResult: any = await dermaResponse.json();

      if (!dermaResult.primaryFinding || dermaResult.confidence == null) {
        return res.status(502).json({ error: 'DermaVision returned a malformed response.' });
      }
      ...
      console.error('Failed to communicate with DermaVision backend:', err.message);
      return res.status(502).json({ error: 'Failed to communicate with DermaVision backend.' });
    }
  });
  ...
  app.post('/api/generate-pdf', async (req, res) => {
    try {
      console.log('Forwarding PDF generation request to DermaVision backend...');
      ...
      if (!djangoResponse.ok) {
        ...
        return res.status(502).json({ error: 'Failed to generate PDF on backend.' });
      }

      const result: any = await djangoResponse.json();
      if (!result.pdf_b64) {
        return res.status(502).json({ error: 'Backend returned malformed PDF.' });
      }
